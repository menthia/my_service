...
tab_write, tab_mine, tab_stats = st.tabs(["기록 남기기", "내 기록", "전체 현황"])

with tab_write:
    st.caption("이름·지역·만족도·메모를 입력해 방문 기록을 남깁니다.")
...
with tab_mine:
    st.caption("이름으로 내가 남긴 기록을 조회하고, 필요하면 삭제합니다.")
...
with tab_stats:
    st.caption("전체 통계와 지역별 만족도, 검색 조건이 적용된 전체 기록을 확인하고 CSV로 내려받습니다.")
